...
from .routes import auth_working as auth, items, search, analytics, admin, utils, files
...
async def lifespan(app: FastAPI):
    ...
    try:
        from .routes.auth_working import get_database
        ...
        logger.info("📝 API will continue without database features")
...
app = FastAPI(
    title="ECO-ALT API",
    description="🌱 AI-Powered Eco-Friendliness Scoring Backend with Database Integration",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc"
...
app.include_router(auth.router)   # Authentication routes (with database)
...
async def app_info():
    ...
    return {
        "name": "ECO-ALT API",
        "version": "1.0.0",
        "description": "AI-powered eco-friendliness scoring backend with database",
...
async def database_status():
    ...
    try:
        ...
        return {
            "status": "connected",
            "database": os.getenv("MONGODB_DATABASE", "eco_products"),
